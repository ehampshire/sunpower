...
def main():
    global cnx, fmt, ignoreTimestamp, PVname
    ...
    parser.add_argument("-i", dest="ignoreTimestamp", action="store_true",
                        help="Ignore timestamp (ie. process all rows in the SP_RAW2 table).")
    parser.add_argument("-s", "-supervisor", dest="PVname", default=None, required=False,
                        help="Optional supervisor serial number to look for.")
    ...
    PVname = config["defaults"]["supervisor"]
    ...
    try:
        ...
        if (ignoreTimestamp):
            #min_timestamp = get_min_timestamp("sp_raw2")
            #print "min_timestamp: %s" % (min_timestamp)

            max_timestamp = get_max_timestamp("sp_raw2")
            ...
            #time_period = get_period_value(PVname)
            ...
            #read_used(yesterdayString)
            #read_produced(yesterdayString)
            read_used(max_timestamp[0])
            read_produced(max_timestamp[0])
            max_timestamp_power = get_max_timestamp("sp_energy")
            read_power(max_timestamp_power[0])
            # read_power(yesterdayString)
        cnx.close()
...
def get_config_path(args):
    ...
    config_file = None
    config_home = path.join(path.expanduser("~/.config/solar_analysis.conf"), CONFIG_FILE_NAME)
...
def get_period_value(supervisor):
    global cnx, fmt
    icursor = cnx.cursor()
    sql = "SELECT energy_total, src_timestamp FROM sp_raw2 WHERE device_serial='%s' order by src_timestamp desc limit 2" % (
        supervisor)
    icursor.execute(sql)
    print(icursor.statement)
    results = icursor.fetchall()
    save_timestamp = None
    for row in results:
        energy_total = row[0]
        src_timestamp = row[1]
        if save_timestamp is not None:
            print "src_timestamp: %s" % (src_timestamp)
            print "save_timestamp: %s" % (save_timestamp)
            # src_timestamp_object = datetime.strptime(src_timestamp, fmt)
            # save_timestamp_object = datetime.strptime(save_timestamp, fmt)
            # time_difference = src_timestamp_object - save_timestamp_object
            time_difference = save_timestamp - src_timestamp
            print "time_difference: %s" % (time_difference)
            time_difference_in_minutes = (save_timestamp - src_timestamp).total_seconds() / 60.0
            print "time_difference_in_minutes: %s" % (time_difference_in_minutes)
        save_timestamp = src_timestamp
...
def insert_line_into_sp_energy(period, per_produced, per_used, per_net, cum_produced, cum_used, cum_net):
    ...
    id = icursor.fetchone()
    if (id is None):
        sql = "INSERT into sp_energy(period, per_produced, per_used, per_net, cum_produced, cum_used, cum_net) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (
        period, per_produced, per_used, per_net, cum_produced, cum_used, cum_net)
    else:
        update = True
        sql = "UPDATE sp_energy SET per_produced='%s', per_used='%s', per_net='%s', cum_produced='%s', cum_used='%s', cum_net='%s' where period='%s'" % (
        per_produced, per_used, per_net, cum_produced, cum_used, cum_net, period)
    # print "sql: %s" % (sql)
    icursor.execute(sql)
    print(icursor.statement)
    id = icursor.lastrowid
...
def read_used(start_timestamp):
    global cnx, ignoreTimestamp, PVname
    ...
    if (ignoreTimestamp):
        sql = "SELECT energy_total, src_timestamp, insert_timestamp FROM `sp_raw2` WHERE device_serial = '%s' order by src_timestamp" % (
            PVname)
    else:
        sql = "SELECT energy_total, src_timestamp, insert_timestamp FROM `sp_raw2` WHERE src_timestamp>='%s' and device_serial = '%s' order by src_timestamp" % (
        start_timestamp, PVname)
    ...
    for row in results:
        ...
        if (last_watts != 0):
            ...
            time_difference_in_minutes = (src_timestamp - last_timestamp).total_seconds() / 60.0
            # print "time_difference_in_minutes: %s" % (time_difference_in_minutes)
            # temper = difference / 0.08333333333;
            # temper = abs(difference / (60 / time_difference_in_minutes))
            temper = difference / (60 / time_difference_in_minutes)
        ...
        if calculated_watts != 0:
            insert_used_line_into_sp_power(insert_timestamp, calculated_watts)
        counter = counter + 1;
...
def read_produced(start_timestamp):
    global cnx, ignoreTimestamp, fmt
    cursor = cnx.cursor(buffered=True)
    sql = "SELECT distinct device_serial FROM `sp_raw2` WHERE device_serial != '%s'" % (PVname)
    # Execute the SQL command
    cursor.execute(sql)
    print(cursor.statement)
    # Fetch all the rows in a list of lists.
    ...
    for i in devices:
        ...
        if (ignoreTimestamp):
            sql = "SELECT ac_power, src_timestamp, insert_timestamp, device_serial FROM `sp_raw2` WHERE device_serial='%s' order by insert_timestamp" % (
                this_device)
        else:
            sql = "SELECT ac_power, src_timestamp, insert_timestamp, device_serial FROM `sp_raw2` WHERE src_timestamp>='%s' and device_serial='%s' order by insert_timestamp" % (
            start_timestamp, this_device)
        # Execute the SQL command
        cursor.execute(sql)
        # print(cursor.statement)
        rowcount = cursor.rowcount
        ...
        for row in results:
            ...
            if (last_watts != 0):
                ...
                # print "time_difference_in_minutes: %s" % (time_difference_in_minutes)
                # temper = difference / 0.08333333333;
                temper = abs(watts / (60 / time_difference_in_minutes))
                this_device_calculated_watts = round(temper, 5);
                calculated_watts = calculated_watts + this_device_calculated_watts
                total_watts = total_watts + calculated_watts
                this_device_total_watts = this_device_total_watts + calculated_watts

            last_watts = watts
            last_timestamp = src_timestamp

            # this_device_total_watts = this_device_total_watts + watts
            # total_watts = total_watts + watts
            counter = counter + 1;
            if insert_timestamp is None:
                continue
            if (src_timestamp_dict.has_key(insert_timestamp)):
                #print "src_timestamp: %s + %s" % (src_timestamp_dict[insert_timestamp], calculated_watts)
                src_timestamp_dict[insert_timestamp] = src_timestamp_dict[insert_timestamp] + calculated_watts
            else:
                #print "src_timestamp: %s" % (calculated_watts)
                src_timestamp_dict[insert_timestamp] = calculated_watts
        # Now print fetched result
        # print "watts=%s : src_timestamp=%s :  insert_timestamp=%s : total_watts=%s : difference=%s : calculated_watts=%s" % (watts, src_timestamp, insert_timestamp, total_watts, difference, calculated_watts)
        print "this_device_total_kilowatts (%s): %s , rowcount: %s" % (this_device, this_device_total_watts, rowcount)
        # print "this_device_calculated_watts (%s): %s" % (this_device, this_device_calculated_watts)
    print "total kW produced: %s" % (total_watts)
    # print "total rows: %s" % (counter)
...
def process_power_rows(start_timestamp, period, period_generated, period_used, period_counter):
    ...
    for period in sorted(period_generated.iterkeys()):
        ...
        print "%s: used - %s , generated - %s, net - %s, cum_produced - %s, cum_used - %s, cum_net - %s" % (
        dbPeriod, period_usedV, period_generatedV, period_net, cum_produced, cum_used, cum_net)
        insert_line_into_sp_energy(dbPeriod, period_generatedV, period_usedV, period_net, cum_produced, cum_used,
                                   cum_net)
